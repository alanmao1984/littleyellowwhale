...
import { repository } from './connection-panels'
...
function ConnectionGuide() {
  ...
  return <div className="flex flex-col gap-5"><Tabs value={os} onValueChange={value => { setOs(String(value)); setCopied(false) }}><TabsList className="w-full"><TabsTrigger value="windows"><Monitor data-icon="inline-start" />Windows</TabsTrigger><TabsTrigger value="macos"><Laptop data-icon="inline-start" />macOS</TabsTrigger></TabsList></Tabs><div className="rounded-xl border bg-background p-4"><div className="flex items-center gap-2"><GitBranch className="size-4" /><span className="font-mono text-sm">alanmao1984/littleyellowwhale</span></div><div className="pt-3"><Badge variant="secondary">{t('待发布可安装版本', 'Installable release pending')}</Badge></div><p className="pt-3 text-sm leading-relaxed text-muted-foreground">{t('这是指定的交付仓库，不代表已有通过验证的节点程序。当前不提供下载或自动执行入口。', 'This is the delivery repository, not a verified node release. Downloads and automatic execution are not enabled.')}</p></div><ol className="flex flex-col gap-4">{[
...
export function WorkspaceDialogs() {
  ...
  return <Dialog open={!!modal} onOpenChange={open => { if (!open) setModal(null) }}><DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>{content[0]}</DialogTitle><DialogDescription>{content[1]}</DialogDescription></DialogHeader>
    ...
    {modal === 'help' && <div className="flex flex-col gap-5">{[
      ...
      [t('关闭网页后，节点还在运行吗？', 'Does closing the web page stop a node?'), t('网页只是控制入口。后台节点必须单独安装并运行；常驻机制完成真机测试前，不承诺关闭客户端后持续接单。', 'The web page is only a control surface. A separate background node must be installed and running. Continued execution is not promised until tested on actual devices.')],
