...
export function TaskDraftForm() {
  ...
  if (prepared) return <div className="flex flex-col gap-5">
    ...
    <ExecutionConsent value={execution} onChange={setExecution} />
  ...
  return <form onSubmit={review}><FieldGroup>
    ...
    {taskType === 'image' && <Field><FieldLabel htmlFor="comfy-workflow">ComfyUI API 工作流 JSON</FieldLabel><Textarea id="comfy-workflow" value={workflow} onChange={event => setWorkflow(event.target.value)} placeholder="粘贴 ComfyUI Save (API Format) 导出的 JSON" required className="min-h-28 font-mono text-xs" /><FieldDescription>{t('工作流中的 prompt、positive 或 text 输入会被每行的 prompt 替换；每行还需包含 outputDir。', 'The first prompt, positive, or text input is replaced by each line prompt; every line must also include outputDir.')}</FieldDescription></Field>}
