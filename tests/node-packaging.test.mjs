...
test("公开仓库未提供接单核心且外部 SEA 构建默认拒绝", async () => {
  const source = await readFile(
    new URL("../scripts/build-node-sea.mjs", import.meta.url),
    "utf8",
  );
  ...
  const build = spawnSync(
    process.execPath,
    [new URL("../scripts/build-node-sea.mjs", import.meta.url).pathname],
    { encoding: "utf8" },
  );
...
test("原生安装器不注册服务或自动启动", async () => {
  const windows = await readFile(
    new URL("../installer/windows/venus-node.iss", import.meta.url),
    "utf8",
  );
  const macos = await readFile(
    new URL("../installer/macos/build-pkg.sh", import.meta.url),
    "utf8",
  );
  assert.doesNotMatch(
    `${windows}\n${macos}`,
    /\b(sc\.exe|launchctl|RunOnce|service)\b/i,
  );
...
test("Pull Request 只验证公开代码，正式构建才读取私有核心", async () => {
  const workflow = await readFile(
    new URL("../.github/workflows/release-node.yml", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    workflow,
    /PRIVATE_CORE_PACKAGE: ["']@alanmao1984\/littleyellowwhale-ant-internal["']/,
  );
  assert.equal(
    workflow.match(/NODE_CORE_READ_TOKEN \|\| github\.token/g)?.length,
    2,
  );
  assert.equal(
    workflow.match(/if: github\.event_name != 'pull_request'/g)?.length,
    3,
  );
